  ...
  implements AfterViewInit, OnChanges, OnDestroy
{
  ...
  @Output("amScrolledToBottom") amScrolledToBottom: Observable<boolean> =
    new Observable();
  ...
  private registerScrollHandlers() {
    ...
    new MutationObserver(() => this.mutations.next(null)).observe(
    ...
    this.scrollEvent = fromEvent(this.self.nativeElement, "scroll").pipe(
      takeUntil(this.destroy$),
      debounceTime(100)
    );
  ...
  private scrollToBottom() {
    setTimeout(
      () =>
        this.self.nativeElement.scrollTo({
          left: 0,
          top: this.self.nativeElement.scrollHeight,
          behavior: "smooth",
        }),
      0
    );
